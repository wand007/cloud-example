 ...
 * @Date 2018/9/18
 */
public enum ThreadPoolEnumSingleton {
    ...
    ThreadPoolEnumSingleton() {
        ThreadFactory nameThreadFactory = new ThreadFactoryBuilder().setNameFormat("example-schedule-pool-%d")
                .build();
        //手动初始化线程池
        mThreadPool = new ThreadPoolExecutor(SIZE_CORE_POOL, SIZE_MAX_POOL, 10L,
                TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>(1000), nameThreadFactory);
    }
